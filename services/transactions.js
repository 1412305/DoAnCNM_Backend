...
module.exports = exports = ({ db, utils, events }) => {
    ...
    // Update block
    let addToBlock = async function (hash, block, index) {
        await db(TABLE_NAME).transacting(block.trx).where('hash', hash).update({
    ...
    let addToTransactionPool = async function (transaction) {
        // Add to database
        await db(TABLE_NAME).transacting(transaction.trx).insert({
        ...
        await db(INPUT_TABLE_NAME).transacting(transaction.trx).insert({
        ...
        await db(OUTPUT_TABLE_NAME).transacting(transaction.trx).insert({
            transactionHash: transaction.hash,
            index: index,
            value: output.value,
